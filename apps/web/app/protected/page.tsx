...
import { AuthProvider, TopBar, useAgilityAuth } from '@agility/auth-tools/components';
import { configureSdkAdapter } from '@agility/auth-tools/adapters';
import { WebsiteAccess, LocaleInfo, ServerUser } from '@agility/auth-tools/models';
import { getMgmtApiClient, getFullMgmtApiClient } from '@agility/auth-tools';
...
const DashboardHero = () => {
  ...
  // Note: Removed auto-selection to prevent redirect loops
  // Users can manually select website and locale from the auth bar
  ...
  useEffect(() => {
    ...
    if (auth.isAuthenticated) {
      if (auth.selectedWebsite && auth.selectedLocale) {
        fetchDashboardData();
      } else {
        console.log('Authenticated but no website/locale selected, showing default data');
        // Show default data when website/locale not selected
        setDashboardData({
          pages: 0,
          contentLists: 0,
          contentItems: 0,
        ...
        <div className="text-center text-white mb-8">
          ...
          <p className="text-purple-100">
            {auth.selectedWebsite ? 
              ...
              'Select a website and locale from the top bar to view data'
            }
          </p>
        </div>
...
const InteractiveExample = () => {
  ...
  const testMgmtClient = async () => {
    setIsTestingClient(true);
    setClientResult('Testing management client...');
    
    try {
      // Test the new management API client
      const client = await getMgmtApiClient();
      console.log('Management client created:', client);
      
      // Test getting current user
      const user = await client.serverUserMethods.me();
      console.log('User from management client:', user);
      
      setClientResult(`✅ Success! Got user: ${user.emailAddress || user.userName || 'Unknown'}`);
    } catch (error) {
      console.error('Management client test failed:', error);
      setClientResult(`❌ Error: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
  ...
  const testFullClient = async () => {
    setIsTestingClient(true);
    setClientResult('Testing full management client...');
    
    try {
      // Test the full management SDK client
      const fullClient = await getFullMgmtApiClient();
      console.log('Full management client created:', fullClient);
      
      // Test getting current user with full client
      const user = await fullClient.serverUserMethods.me();
      console.log('User from full client:', user);
      
      setClientResult(`✅ Full Client Success! User: ${user.emailAddress || 'Unknown'}`);
    } catch (error) {
      console.error('Full client test failed:', error);
      setClientResult(`❌ Full Client Error: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setIsTestingClient(false);
    }
  };
    ...
    <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700">
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <button
            onClick={testMgmtClient}
            disabled={isTestingClient || !auth.isAuthenticated}
            className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            {isTestingClient ? 'Testing...' : 'Test getMgmtApiClient()'}
          </button>
          
          <button
            onClick={testFullClient}
            disabled={isTestingClient || !auth.isAuthenticated}
            className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            {isTestingClient ? 'Testing...' : 'Test getFullMgmtApiClient()'}
          </button>
        </div>
          ...
          <div className="flex items-center justify-between p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
            ...
            <button
              onClick={async () => {
                try {
                  await auth.signOut();
                  // Redirect back to main page after sign out
...
// No client-side route protection needed - handled by middleware
              ...
              <div className="space-y-8">
                ...
                <div className="border-l-4 border-green-500 pl-6">
                  <div className="flex items-center mb-4">
                    ...
                    <h3 className="text-xl font-semibold text-gray-900 dark:text-gray-100">Authentication Status</h3>
                  </div>
                  <InteractiveExample />
                </div>
              </div>
